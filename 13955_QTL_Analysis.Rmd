...
format_qtlmap_geno<-function (bed, 
  ...
  parents <- data.frame(snp = rownames(genomat), a = genomat[[par_a]], 
      b = genomat[[par_b]], stringsAsFactors = FALSE)
  parents$sum <- parents$a + parents$b
  drop_snps <- NULL
  drop_snps <- union(drop_snps, parents$snp[parents$a == parents$b])
  if (rm_het) {
      drop_snps <- union(drop_snps, parents$snp[parents$a == 
          1 | parents$b == 1])
  }
  else {
      drop_snps <- union(drop_snps, parents$snp[is.na(parents$a) & 
          parents$b == 1])
      drop_snps <- union(drop_snps, parents$snp[parents$a == 
          1 & is.na(parents$b)])
  }
  ...
  flip <- union(flip, parents$snp[is.na(parents$sum) & parents$b == 
      0])
  flip <- union(flip, parents$snp[parents$sum == 1 & parents$b == 
      0])
  ...
  if (out_fmt == "icimapping") {
      ...
      id_df <- bed@snps[c("id", "chr", "dist", "pos", "A1", 
          "A2")]
...
setwd("C:/Users/zwinn/OneDrive/Publication/Manuscripts_In_Progress/FHB QTL/Final Formatted Data and Analysis")
...
for(i in env){
  for(j in traits){
    ...
    if(j=="FHB" | j=="FDK" | j=="DON"){
      ...
      if(fit$converge==FALSE | c(i=="WAR20" & j=="FHB")){
...
marker_count<-c() #set up list for chromosome lengths

for (i in cross_file$geno){
  marker_count <- c(marker_count, length(i$map))
}

#create data frame with number of markers per linkage group
marker_count<-data.frame(Chr = names(cross_file$geno), 
                         n = marker_count) %>%
  filter(marker_count>6 &
           Chr!='1A.2' &
           Chr!='6A.2')
...
# write function to look at cM vs BP position
compare_bp_to_cm <- function(x){
  # Pull marker names
  mn<-markernames(x)
  
  # find those markers positions
  pos<-find.markerpos(x,mn)
  
  # Get the marker name in the dataframe
  pos<-rownames_to_column(pos,var="BP_Location")
  
  # separate those markers
  ...
  # bind together the marker names and position
  pos<-cbind(mn,pos)
  
  # Rename columns
  colnames(pos)<-c("Markers", "Chr", "BP", "cM")
  
  # Make sure those last two columns are numeric
...
for (i in names(cross_file$geno)){
  # subset the total data
  q<-pos %>% filter(Chr==i)
  
  # if the first position is greater than the last position 
  if (q[1,3]>q[(nrow(q)),3]){
    # flip the chromosome 
